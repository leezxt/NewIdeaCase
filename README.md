...
## 實際畫面 / Verified screenshots

以下畫面擷取自本機 Docker Compose 環境，並在擷取前確認核心 API、Dashboard、Swagger、Prometheus 與 Grafana 服務可正常回應。

The following screenshots were captured from the local Docker Compose environment after verifying that the core API, operations dashboard, Swagger UI, Prometheus, and Grafana were responding successfully.

### 營運 Dashboard / Operations dashboard

應用健康狀態、HTTP 請求、成功率、回應時間、JVM 資源與知識匯入 queue 的即時摘要。

Live application health, HTTP traffic, success rate, response time, JVM resources, and knowledge-ingestion queue status.

![NewIdeaCase operations dashboard](docs/images/operations-dashboard.png)

### API 文件 / OpenAPI documentation

由應用程式實際產生的 Swagger UI，列出使用者、RAG、商品、訂單與知識文件 API。

Swagger UI generated by the running application, covering user, RAG, catalog, order, and knowledge-document APIs.

![NewIdeaCase Swagger API](docs/images/swagger-api.png)

### Grafana 監控 / Grafana monitoring

由 Prometheus datasource 驅動的 availability、request rate、error ratio、p95 latency、JVM heap 與 ingestion 指標。

Prometheus-backed availability, request rate, error ratio, p95 latency, JVM heap, and ingestion metrics.

![NewIdeaCase Grafana monitoring](docs/images/grafana-monitoring.png)
